...
- [x] `settings.toml` 및 `settings.toml.example`의 `[telegram].storage_dir` 기본값이 `G:\내 드라이브\투자\Asset_jun_bot_storage`로 동기화된다.
- [x] `scripts/get_storage_dir.py` 및 관련 설정 로더가 공백, 따옴표, 백슬래시가 포함된 경로를 안정적으로 반환한다.
- [x] 기존 테스트 세트에서 경로 관련 설정 단위 테스트가 정상 통과한다.
