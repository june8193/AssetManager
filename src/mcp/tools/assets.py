...
async def _append_performance_metrics(target_dict: dict) -> dict:
    """응답 딕셔너리에 백엔드 성과 지표(Sharpe, Sortino, MDD)를 병합하는 헬퍼 함수"""
    if isinstance(target_dict, dict) and "error" not in target_dict:
        perf = await api_client.get("/api/v1/performance/portfolio", params={"period": "1Y"})
        if isinstance(perf, dict) and "sharpe_ratio" in perf:
            target_dict["sharpe_ratio"] = perf.get("sharpe_ratio", 0.0)
            target_dict["sortino_ratio"] = perf.get("sortino_ratio", 0.0)
            target_dict["mdd"] = perf.get("mdd", 0.0)
            target_dict["max_mdd"] = perf.get("max_mdd", 0.0)
    return target_dict
...
async def get_asset_ratios() -> dict:
    ...
    try:
        ...
        return await _append_performance_metrics(result)
    except Exception as e:
...
async def get_portfolio_status(date: Optional[str] = None) -> dict:
    ...
    try:
        ...
        return await _append_performance_metrics(status)
    except Exception as e:
