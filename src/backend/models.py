...
class AllocationSetting(Base):
    """자산배분 파라미터 설정 및 시뮬레이션 결과를 저장하는 모델입니다.
    
    Attributes:
        id (int): 고유 식별자 (PK)
        name (str): 설정 명칭 (예: '기본 보수적 설정')
        description (str): 설명 (선택)
        target_index (str): 대상 지수 (KOSPI, KOSDAQ, S&P500, NASDAQ)
        lookback_period (int): 기준 기간 (이동평균/모멘텀)
        rebalancing_frequency (str): 리밸런싱 주기 (매일, 매월 말, 매 분기 말)
        vix_threshold (float): VIX 임계값
        min_cash_weight (float): 최소 현금 비중 (%)
        max_cash_weight (float): 최대 현금 비중 (%)
        start_date (str): 시뮬레이션 시작 날짜 (YYYY-MM-DD)
        end_date (str): 시뮬레이션 종료 날짜 (YYYY-MM-DD)
        is_favorite (bool): 주로 참고할 설정 여부
        simulation_result (str): 시뮬레이션 결과 JSON 문자열 (빠른 로딩을 위한 캐시)
        created_at (datetime): 생성 일시
        updated_at (datetime): 수정 일시
    """
    ...
    target_index = Column(String, nullable=False)
    lookback_period = Column(Integer, nullable=False)
    rebalancing_frequency = Column(String, nullable=False)
    vix_threshold = Column(Float, nullable=False)
    min_cash_weight = Column(Float, nullable=False)
    max_cash_weight = Column(Float, nullable=False)
    start_date = Column(String, nullable=False)
    end_date = Column(String, nullable=True)
    is_favorite = Column(Boolean, default=False, nullable=False)
    simulation_result = Column(String, nullable=True) # JSON String 캐시
    created_at = Column(DateTime, default=datetime.datetime.now)
