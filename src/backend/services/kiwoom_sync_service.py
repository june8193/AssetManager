...
class KiwoomTransactionService:
    ...
    async def sync_transactions(self, db: Session, days: int = 7) -> dict:
        ...
        # 1. DB에서 키움증권 계좌 획득
        account = db.query(Account).filter(Account.provider == "키움증권", Account.is_active == True).first()
        if not account:
        ...
        try:
            ...
            if success_count > 0:
                db.commit()
                logger.info(f"거래내역 동기화 성공: 총 {success_count}건 저장 완료.")
            ...
            db.rollback()
            logger.error(f"거래내역 동기화 작업 중 오류 발생: {str(e)}")
            raise e
