...
class PerformanceService:
    ...
    def calculate_portfolio_performance(self, period: str = "1Y") -> Dict:
        ...
        for i in range(1, len(rows)):
            ...
            # 기하 평균 일일 보간 수익률: (1 + r_k)^(1 / delta_t) - 1
            if 1 + r_k > 0:
        ...
        # TWR 누적 지수 및 Drawdown 연산
        twr_index = 100.0
        peak = twr_index
        drawdown_series = []
        max_mdd = 0.0

        for d, r in zip(daily_dates, daily_returns):
            twr_index = twr_index * (1.0 + r)
            if twr_index > peak:
                peak = twr_index
            dd = ((twr_index - peak) / peak) * 100.0 if peak > 0 else 0.0
            if dd < max_mdd:
                max_mdd = dd
            drawdown_series.append({"date": d.strftime("%Y-%m-%d"), "drawdown": round(dd, 2)})
