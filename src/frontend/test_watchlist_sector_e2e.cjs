...
async function main() {
  ...
  // 브라우저 얼럿 다이얼로그 처리 (자동 dismiss)
  page.on('dialog', async dialog => {
    console.log(`[E2E] Dialog detected: [${dialog.type()}] "${dialog.message()}". Dismissing...`);
  ...
  // 3. 종목 검색 테스트 (중복 데이터가 없도록 '삼성전자우' / '005935' 검색)
  console.log("[E2E] Searching for '005935'...");
  await page.fill('input[placeholder*="종목명 또는 6자리 종목코드를 입력하세요"]', '005935');
  ...
  // 4. 관심종목 등록 테스트
  console.log("[E2E] Registering '삼성전자우' to watchlist...");
  ...
  // 5. 커스텀 섹터에 종목 추가 테스트
  console.log("[E2E] Adding '삼성전자우' to sector 'E2E 반도체'...");
