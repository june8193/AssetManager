...
const mockStats = {
  ...
  category_breakdown: [
    { category_id: 1, category_name: '식비/카페', color: '#FF6B6B', amount: 40000, percentage: 40.0 },
    { category_id: 2, category_name: '쇼핑', color: '#4ECDC4', amount: 30000, percentage: 30.0 },
    { category_id: 4, category_name: '구독료', color: '#8B5CF6', amount: 30000, percentage: 30.0 },
  ],
...
describe('ExpensesPage 컴포넌트 테스트', () => {
  ...
  it('통계 제외 거래(is_excluded=True)는 원장 테이블에서 카테고리 셀렉트가 disabled 처리되어야 한다', async () => {
    renderComponent();

    await waitFor(() => {
      expect(screen.getByText('스타벅스 강남점')).toBeInTheDocument();
      expect(screen.getByText('현대카드 결제대금')).toBeInTheDocument();
    });

    const selects = screen.getAllByRole('combobox');
    // 테이블 내의 통계 제외 거래(현대카드 결제대금) 카테고리 셀렉트 찾기
    // id: 103 거래는 is_excluded: true, category_id: null
    const excludedSelect = selects.find((sel) => sel.value === '' && sel.disabled);
    expect(excludedSelect).toBeDefined();
    expect(excludedSelect).toBeDisabled();

    // 정상 유효 지출(스타벅스 강남점, id: 101)의 셀렉트는 enabled 상태여야 함
    const includedSelect = selects.find((sel) => sel.value === '1');
    expect(includedSelect).toBeDefined();
    expect(includedSelect).not.toBeDisabled();
  });

  it('2차 카테고리 열 및 필터가 제거되고 단일 카테고리 필터 선택 시 해당 category_id로 거래 내역을 조회해야 한다', async () => {
    renderComponent();
    ...
    // 테이블 헤더에서 '2차 카테고리' 열이 제거되었는지 확인
    expect(screen.queryByText('2차 카테고리')).toBeNull();

    // 2차 카테고리 필터 셀렉트박스가 존재하지 않아야 함
    const selects = screen.getAllByRole('combobox');
    const hasSubCatFilter = selects.some((sel) =>
      Array.from(sel.options).some((opt) => opt.text.includes('2차 카테고리'))
    );
    expect(hasSubCatFilter).toBe(false);

    // 단일 카테고리 필터 셀렉트박스 찾기
    const catFilterSelect = selects.find((sel) =>
      Array.from(sel.options).some((opt) => opt.text === '카테고리 전체')
    );
    expect(catFilterSelect).toBeDefined();

    // '식비/카페'(id: 1) 선택
    fireEvent.change(catFilterSelect, { target: { value: '1' } });
  ...
  it('대시보드 상단에 2차 카테고리(지출 특성) 요약 배너가 노출되지 않고, 단일 카테고리(구독료 등) 비중이 정상 렌더링되어야 한다', async () => {
    renderComponent();

    await waitFor(() => {
      // 상단 지출 특성 요약 배너는 제거되어 보이지 않아야 함
      expect(screen.queryByText(/지출 특성 요약/i)).toBeNull();
      // 카테고리별 비중에 '식비/카페', '쇼핑' 등이 정상 노출됨
