...
- 평일(`is_holiday == False`):
  ...
  2. **마감 수치 및 등락률 계산 로직**:
     ...
     - **3대 지수(`^GSPC`, `^IXIC`, `^DJI`) 변동폭 및 등락률 계산**:
       - 변동폭(Change) = C_today - C_prev
       - 등락률(%) = ((C_today - C_prev) / C_prev) * 100
       - 부호 표기 원칙: 상승 시 `+` 부호 필수 기재 (예: `+1.23%`, `+50.25pt`), 하락 시 `-` 부호 (예: `-0.45%`, `-12.30pt`), 보합 시 `0.00%`, `0.00pt`
     - **VIX 지수(`^VIX`) 변동폭 계산**:
       - VIX는 변동률(%)의 의미가 없으므로 **전일대비 % 등락률은 따지지 않으며**, 당일 마감 수치(레벨)와 전일 대비 변동폭(pt = C_today - C_prev)만 산출합니다.
  3. **VIX 4단계 리스크 등급 분류**:
