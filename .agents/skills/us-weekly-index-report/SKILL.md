...
3. **주간 VIX 데이터 분석 로직 (High-Low Range 및 리스크 평가)**:
   - VIX는 지수와 달리 등락률(%)의 의미가 없으므로 **주간 % 변동률은 따지지 않으며**, 주간 마감 종가 및 주간 시작일 대비 변동폭(pt = 종료가 - 시작가)만 산출합니다.
...
- **완료 검증 조건 (Completion Criterion)**:
  ...
  - [ ] 3대 지수 주간 변동률 및 VIX 주간 변동폭(pt) 계산 완료
...
- **서식 규칙**:
  ...
  - VIX 모니터링은 독립된 섹션으로 구성하여 주간 마감가, 변동폭(pt), 주간 변동 범위(최저~최고), 리스크 등급 이모지(🟢/🟡/🟠/🔴) 및 공포/안정 추세 진단을 명확히 표시합니다. (VIX는 % 변동률 미기재)
- **보고서 템플릿 양식**:
```markdown
