...
async def test_update_all_market_prices_exchange_trigger(db_session: Session):
    """오전 9시 30분 이후 영업일에 당일 환율 정보가 없으면 환율 수집이 트리거되는지 검증합니다."""
    ...
    # 2. 오전 9시 30분 이후(오전 9시 30분) 실행 & DB에 오늘 환율이 없을 때 -> 환율 수집 실행
    ...
    with patch.object(price_service, "fetch_and_save_exchange_rate", mock_fetch), \
        ...
        # 9시 30분 이후이므로 호출되어야 함
        assert mock_fetch.called

    # 3. 오전 9시 30분 이후 실행 & DB에 이미 오늘 환율이 있을 때 -> 환율 수집 스킵
    mock_fetch.reset_mock()
    # DB에 오늘 환율 미리 주입
    ...
    # 4. 수동 새로고침 (is_manual=True) 일 때 -> 오전 9시 30분 이후여도 환율 수집 미실행
    mock_fetch.reset_mock()
