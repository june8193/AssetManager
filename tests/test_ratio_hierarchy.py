...
async def test_get_ratio_hierarchy_api(db_session: Session):
    ...
    asset1 = Asset(ticker="005930", name="삼성전자", major_category="주식", sub_category="알파(성장)", country="KR")
    asset2 = Asset(ticker="AAPL", name="애플", major_category="주식", sub_category="코어(지수)", country="US")
    db_session.add_all([asset1, asset2])
    db_session.flush()
    
    # 목표 비중 설정
    db_session.add(TargetRatio(category_name="주식", category_type="major", target_percentage=100.0))
    db_session.add(TargetRatio(category_name="알파(성장)", category_type="sub", target_percentage=60.0, parent_category="주식"))
    db_session.add(TargetRatio(category_name="코어(지수)", category_type="sub", target_percentage=40.0, parent_category="주식"))
    ...
    assert "알파(성장)" in sub_names
    assert "코어(지수)" in sub_names
    
    # 국내주식 하위 자산 확인
    domestic = next(child for child in major["children"] if child["category_name"] == "알파(성장)")
...
def test_bulk_update_target_ratios(db_session: Session):
    ...
    sub = next(t for t in targets if t.category_name == "알파(성장)")
