...
def test_asset_service_crud_lifecycle(db_session):
    ...
    assert created.major_category == "주식"
    assert created.sub_category == "알파(성장)"
    ...
    assert updated.major_category == "주식"
    assert updated.sub_category == "배당주"
...
async def test_asset_service_verify_asset(db_session):
    ...
    with patch("src.backend.services.asset_service.price_service.get_stock_name", new_callable=AsyncMock) as mock_get_name:
        ...
        stock_result = await service.verify_asset(ticker="005930", country="KR", major_category="주식")
    ...
    with patch("src.backend.services.asset_service.price_service.get_stock_name", new_callable=AsyncMock) as mock_get_name:
        ...
        with pytest.raises(LookupError) as excinfo:
            await service.verify_asset(ticker="INVALID", country="US", major_category="주식")
    ...
    with pytest.raises(ValueError) as excinfo:
        await service.verify_asset(ticker="TSLA", country="US", major_category="주식")
    assert "이미 등록된 자산(티커)입니다" in str(excinfo.value)
