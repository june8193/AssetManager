...
2. **시세 및 환율 자동 업데이트 (Automation)**
   - 키움증권 Open API 및 환율 API를 연동하여 백그라운드 비동기 루프(`lifespan` 내 1시간 주기 실행)를 통해 실시간 자산 평가액 반영.
3. **금융 거래 내역 관리 및 파싱 (Transaction Management)**
   - 각 금융사에서 다운로드한 거래내역 CSV 파일을 브라우저로 업로드 시, 자동으로 파싱하여 데이터베이스에 트랜잭션 기록 및 일괄 반영.
4. **배당금 추적 및 시각화 (Dividend Tracking)**
   - 종목별 배당금 지급 주기 및 연도/월별 배당금 흐름 그래프 제공으로 중장기 현금흐름 추적.
5. **데이터 무결성 관리 (Database Administration)**
