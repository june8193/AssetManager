...
- [ ] 전역 테스트 setup에 `ResizeObserver` 및 윈도우 레이아웃 mock을 표준화한다.
- [ ] Recharts의 `ResponsiveContainer` 및 관련 차트 컨테이너를 가볍게 렌더링되도록 mock 처리한다.
- [ ] 차트가 포함된 대시보드/페이지 테스트 실행 시 차트 크기 경고(The width(-1) and height(-1) of chart should be greater than 0)가 완전히 사라지고 기존 assertion이 100% 통과하는지 검증한다.
