...
- [ ] `DashboardPage` 등에서 백그라운드 태스크/SSE/시스템 상태 polling 네트워크 fetch를 적절히 mock 처리하여 `ECONNREFUSED` 로그 및 대기 지연을 제거한다.
- [ ] 상태 갱신이 비동기로 발생하는 주요 페이지 테스트에서 비동기 전이 처리를 보완하여 `act(...)` 경고를 제거한다.
- [ ] 관련 페이지 테스트들이 격리된 환경에서 완벽히 통과하는지 검증한다.
