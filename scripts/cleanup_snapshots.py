...
def cleanup():
    ...
    try:
        ...
        # 잔액이 동일함이 확인되었으므로, 같은 날짜/계좌 중 입금액(period_deposit)이 0인 행을 삭제합니다.
        # 단, 만약 둘 다 0이더라도 하나는 남겨야 하므로 ID 기반으로 정리합니다.
        
        # 삭제 대상 ID 식별: 동일 (account_id, snapshot_date) 중 
        # period_deposit이 0인 행을 삭제하되, 만약 모든 중복행이 0이면 가장 큰 ID만 남김.
        ...
        cursor.execute("""
            ...
            WHERE id IN (
                ...
                WHERE a1.id != a2.id 
                AND a1.period_deposit = 0.0 
                AND a2.period_deposit != 0.0
            )
        """)
